...
function reverseString(str) {
  let reversed = '';
  for (let i = str.length - 1; i >= 0; i -= 1) reversed += str[i];
  return reversed;
}
...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(list[i])) count += 1;
  }
  return count;
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/[a-z]/gi, (i) => rot13[alphabet.indexOf(i)]);
}
